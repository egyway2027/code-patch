...
export default function CodePatcher(){
 ...
 return <main style={{minHeight:'100vh',background:C.bg,color:C.text,fontFamily:'system-ui,sans-serif',padding:18,direction:rtl?'rtl':'ltr'}}><div style={{maxWidth:1400,margin:'auto'}}>
  ...
  {view==='editor'&&<><section style={{...box,marginBottom:12,display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:10}}><label>اسم الملف<input value={f.fileName} onChange={e=>update('fileName',e.target.value)} style={input}/></label><label>مسار المشروع (project commit)<input value={f.filePath||''} onChange={e=>update('filePath',e.target.value)} style={input}/></label><label>النوع<select value={detectFileType(f.fileName)} onChange={e=>update('fileName',f.fileName.replace(/\.[^.]+$/,'')+'.'+({javascript:'js',typescript:'ts',python:'py',java:'java',c:'c',cpp:'cpp',go:'go',json:'json',html:'html',css:'css'}[e.target.value]||'txt'))} style={input}>{FILE_OPTIONS.map(([v,n])=><option key={v} value={v}>{n}</option>)}</select></label><label>عدد الأحرف<div style={metric}>{sizeInfo(f.content).chars.toLocaleString()} / {LIMITS.maxSourceChars.toLocaleString()}</div></label></section>
  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(350px,1fr))',gap:12}}><section style={box}><h3>الملف الأصلي / Editor</h3><textarea value={f.content} onChange={e=>{update('content',e.target.value);setResult('')}} style={mono}/><input ref={inputRef} hidden type="file" onChange={e=>{const file=e.target.files?.[0];if(file){file.text().then(s=>{update('content',s);update('fileName',file.name)})}}}/><button style={btn(false)} onClick={()=>inputRef.current?.click()}><Upload size={15}/>رفع ملف</button></section><section style={box}><h3>تعديلات SEARCH / REPLACE</h3><textarea value={f.patchText} onChange={e=>update('patchText',e.target.value)} style={mono}/><div style={metric}>الكتل: {parsed.blocks.length} • الأخطاء: {parsed.errors.length}</div></section></div>
