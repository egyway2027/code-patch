...
const mono={width:'100%',minHeight:280,resize:'vertical',background:'#031f1b',color:'#f8f2e9',border:`1px solid ${C.border}`,borderRadius:14,padding:14,fontFamily:'ui-monospace,SFMono-Regular,Menlo,Consolas,monospace',fontSize:13,lineHeight:1.55,direction:'ltr',textAlign:'left',outline:'none'};
 ...
 return <main style={{minHeight:'100vh',background:C.bg,color:C.text,fontFamily:'system-ui,sans-serif',padding:18,direction:rtl?'rtl':'ltr'}}><div style={{maxWidth:1400,margin:'auto'}}>
  <header style={{...box,display:'flex',alignItems:'center',justifyContent:'space-between',gap:18,padding:'20px 22px',borderRadius:28,marginBottom:18}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
      ...
      <button style={{color:'#17342c',border:0,background:'linear-gradient(135deg,#f1c993,#d9ae78)',borderRadius:14,padding:'10px 15px',cursor:'pointer',fontWeight:800,fontSize:12,boxShadow:'0 8px 22px rgba(211,164,111,.18)'}} onClick={analyze}>تحليل المشروع</button>
    </div>
  </header>
  <nav style={{display:'flex',gap:8,marginBottom:18,overflowX:'auto',paddingBottom:2}}>
    <button style={{...btn(false),background:view==='editor'?'linear-gradient(135deg,#e4bc87,#c99660)':'rgba(8,55,47,.72)',color:view==='editor'?'#18362e':'#c9d8d3',borderColor:view==='editor'?'transparent':C.border,fontWeight:view==='editor'?800:600}} onClick={()=>setView('editor')}>🛡️ الأمان</button>
    <button style={{...btn(false),background:view==='analysis'?'linear-gradient(135deg,#e4bc87,#c99660)':'rgba(8,55,47,.72)',color:view==='analysis'?'#18362e':'#c9d8d3',borderColor:view==='analysis'?'transparent':C.border,fontWeight:view==='analysis'?800:600}} onClick={analyze}>⌘ IDE</button>
