...
export default function CodePatcher(){
 ...
 return <main style={{minHeight:'100vh',background:C.bg,color:C.text,fontFamily:'system-ui,sans-serif',padding:'14px 16px',direction:rtl?'rtl':'ltr',width:'100%',boxSizing:'border-box',overflowX:'hidden'}}><div style={{width:'100%',maxWidth:'100%',margin:'0 auto'}}>
  ...
  <section style={{position:'relative',overflow:'hidden',marginBottom:18,padding:24,borderRadius:30,background:'linear-gradient(135deg,rgba(15,83,68,.96),rgba(6,48,41,.97))',border:`1px solid ${C.border}`,boxShadow:'0 22px 60px rgba(0,0,0,.32)'}}>
    <div style={{position:'relative',zIndex:1,display:'flex',justifyContent:'space-between',alignItems:'flex-end',gap:20,flexWrap:'wrap'}}>
      <div>
        ...
        <h2 style={{margin:'7px 0 6px',fontSize:26}}>{t.smartEditorTitle || 'Smart Patch Editor'}</h2>
        <p style={{margin:0,color:'#b6cbc5',fontSize:13,maxWidth:650,lineHeight:1.7}}>{t.smartEditorDesc || 'Luxury workspace with clear separation.'}</p>
      </div>
      <div style={{minWidth:170,padding:'14px 16px',borderRadius:19,background:'rgba(2,27,23,.42)',border:`1px solid ${C.border}`}}>
        <small style={{display:'block',color:C.sub,marginBottom:7}}>{t.systemStatus || 'System status'}</small>
        <strong style={{color:'#e8c48f'}}>● {t.readyReview || 'Ready for review'}</strong>
      </div>
    </div>
  </section>
  ...
  {view==='editor'&&<><section style={{...box,marginBottom:12,display:'grid',gridTemplateColumns:'1.4fr 1.2fr 0.8fr 0.6fr',gap:10,width:'100%',boxSizing:'border-box'}}><label>{t.fileNameLabel || 'File name'}<input value={f.fileName} onChange={e=>update('fileName',e.target.value)} style={input}/></label><label>{t.workspacePathLabel || 'Workspace path'}<input value={f.filePath||''} onChange={e=>update('filePath',e.target.value)} style={input}/></label><label>{t.fileTypeLabel || 'Type'}<select value={detectFileType(f.fileName)} onChange={e=>update('fileName',f.fileName.replace(/\.[^.]+$/,'')+'.'+({javascript:'js',typescript:'ts',jsx:'jsx',tsx:'tsx',python:'py',java:'java',c:'c',h:'h',cpp:'cpp',hpp:'hpp',go:'go',json:'json',html:'html',css:'css'}[e.target.value]||'txt'))} style={input}>{FILE_OPTIONS.map(([v,n])=><option key={v} value={v}>{n}</option>)}</select></label><label>{t.charsLabel || 'Chars'}<div style={metric}>{sizeInfo(f.content).chars.toLocaleString()} / {LIMITS.maxSourceChars.toLocaleString()}</div></label></section>
  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,width:'100%',boxSizing:'border-box'}}><section style={{...box,minWidth:0}}><h3>{t.originalEditor || 'Original / Editor'}</h3><textarea value={f.content} onChange={e=>{update('content',e.target.value);setResult('')}} style={mono}/><input ref={inputRef} hidden type="file" onChange={e=>{const file=e.target.files?.[0];if(file){file.text().then(s=>{update('content',s);update('fileName',file.name)})}}}/><button style={btn(false)} onClick={()=>inputRef.current?.click()}><Upload size={15}/>{t.uploadFile || 'Upload file'}</button></section><section style={{...box,minWidth:0}}><h3>{t.patchSectionTitle || 'SEARCH / REPLACE Patch'}</h3><textarea value={f.patchText} onChange={e=>update('patchText',e.target.value)} style={mono}/><div style={metric}>{t.blocksMetric || 'Blocks'}: {parsed.blocks.length} • {t.errorsMetric || 'Errors'}: {parsed.errors.length}</div></section></div>
