...
import {Languages,FolderOpen,Upload,Play,Copy,Download,Undo2,RefreshCw,Network,Bug,FileCode2,XCircle,CheckCircle2,Plus,Trash2,Sparkles,ShieldAlert} from 'lucide-react';
...
const box={background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:10,boxShadow:'0 8px 20px rgba(0,0,0,.18)'};
const mono={width:'100%',maxWidth:'100%',boxSizing:'border-box',minHeight:0,resize:'none',background:'#031f1b',color:'#f8f2e9',border:`1px solid ${C.border}`,borderRadius:14,padding:8,fontFamily:'ui-monospace,SFMono-Regular,Menlo,Consolas,monospace',fontSize:12,lineHeight:1.4,direction:'ltr',textAlign:'left',outline:'none'};
...
export default function CodePatcher(){
 const isLocalHost=typeof window!=='undefined'&&(window.location.hostname==='localhost'||window.location.hostname==='127.0.0.1'); const compilerServerUrl=import.meta.env.VITE_COMPILER_SERVER_URL || (isLocalHost?'http://127.0.0.1:8787':'');
 ...
 useEffect(()=>{document.documentElement.lang=language;document.documentElement.dir=rtl?'rtl':'ltr'; if(compilerServerUrl){compilerHealth(compilerServerUrl).then(x=>setCompilerOnline(x.ok===true)).catch(()=>setCompilerOnline(false));}else{setCompilerOnline(false);}},[language,rtl,compilerServerUrl]);
 ...
 const run=async()=>{if(!f.content)return setStatus({ok:false,message:t.emptyCode});if(!f.patchText.trim()||parsed.errors.length||!parsed.blocks.length)return setStatus({ok:false,message:t.invalidPatch});setBusy(true);setStatus({ok:true,message:t.tx});try{
   ...
   let data;
   const useServer = compilerTypes.has(fileType) && compilerOnline;
   if(useServer){
     data=await patchWithCompilerServer({id:`v15-${Date.now()}`,original:f.content,patchText:f.patchText,fileName:f.fileName,fileType,mode:MATCH_MODES.EXACT_UNIQUE,allowReviewApply:reviewApproved,reviewApproved},{baseUrl:compilerServerUrl});
   } else {
     data=await new Promise((resolve,reject)=>{const w=new Worker(new URL('./patchWorker.js',import.meta.url),{type:'module'});const id=`v15-${Date.now()}`;w.onmessage=e=>{if(e.data.id===id){w.terminate();resolve(e.data)}};w.onerror=e=>{w.terminate();reject(e)};w.postMessage({id,original:f.content,patchText:f.patchText,fileName:f.fileName,fileType,mode:MATCH_MODES.EXACT_UNIQUE,allowReviewApply:reviewApproved,reviewApproved})});
   }
 ...
 const applyProject=async()=>{if(!compilerOnline)return setProjectResult({ok:false,message:'سيرفر المترجم غير متصل؛ معاملات المشروع تتطلب تشغيل السيرفر.'});setBusy(true);try{const before=files.map(x=>({...x}));const r=await applyProjectWithServer(files.map(x=>({...x,filePath:x.filePath||''})),{baseUrl:compilerServerUrl,mode:MATCH_MODES.EXACT_UNIQUE,reviewApproved});setProjectResult(r);if(r.ok){setServerProjectTx(r);setHistory(h=>[...h,{kind:'project',filesBefore:before}].slice(-20));setFiles(fs=>fs.map((x,i)=>({...x,content:r.results[i]?.code??x.content})));setActive(0);setResult(r.results[0]?.code??'');setReviewApproved(false);}}catch(e){setProjectResult({ok:false,message:e?.message||'Project server error.'})}finally{setBusy(false)}};
 ...
 return <main style={{height:'100vh',background:C.bg,color:C.text,fontFamily:'system-ui,sans-serif',padding:'8px 12px',direction:rtl?'rtl':'ltr',width:'100%',boxSizing:'border-box',overflow:'hidden'}}><div style={{width:'100%',maxWidth:'100%',margin:'0 auto',height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
  <header style={{...box,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,padding:'6px 14px',borderRadius:14,marginBottom:6,flex:'0 0 auto'}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
      ...
      <span style={{...metric,padding:'10px 13px',borderRadius:14}}>Compiler: <b style={{color:compilerOnline?C.green:C.gold2}}>{compilerOnline?t.compilerServerOn:(!compilerServerUrl?(rtl?'وضع السحاب (Cloud)':'Cloud Mode'):t.compilerServerOff)}</b></span>
      <button style={{color:'#17342c',border:0,background:'linear-gradient(135deg,#f1c993,#d9ae78)',borderRadius:14,padding:'10px 15px',cursor:'pointer',fontWeight:800,fontSize:12,boxShadow:'0 8px 22px rgba(211,164,111,.18)'}} onClick={projectAnalyze}>{t.analyzeProject || 'Analyze project'}</button>
    </div>
  </header>
  <nav style={{display:'flex',gap:8,marginBottom:6,overflowX:'auto',paddingBottom:2,flex:'0 0 auto'}}>
    <button style={{...btn(false),background:view==='editor'?'linear-gradient(135deg,#e4bc87,#c99660)':'rgba(8,55,47,.72)',color:view==='editor'?'#18362e':'#c9d8d3',borderColor:view==='editor'?'transparent':C.border,fontWeight:view==='editor'?800:600}} onClick={()=>setView('editor')}>{t.navEditor || 'Editor'}</button>
    <button style={{...btn(false),background:view==='analysis'?'linear-gradient(135deg,#e4bc87,#c99660)':'rgba(8,55,47,.72)',color:view==='analysis'?'#18362e':'#c9d8d3',borderColor:view==='analysis'?'transparent':C.border,fontWeight:view==='analysis'?800:600}} onClick={analyze}>{t.navAnalysis || 'Security & IDE'}</button>
  ...
  <section style={{position:'relative',overflow:'hidden',marginBottom:6,padding:'6px 14px',borderRadius:12,background:'linear-gradient(135deg,rgba(15,83,68,.96),rgba(6,48,41,.97))',border:`1px solid ${C.border}`,flex:'0 0 auto'}}>
    <div style={{position:'relative',zIndex:1,display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,flexWrap:'wrap'}}>
      <h2 style={{margin:0,fontSize:14}}>{t.smartEditorTitle || 'Smart Patch Editor'} <span style={{color:C.gold2,fontSize:9,fontWeight:800}}>V23</span></h2>
      <strong style={{color:'#e8c48f',fontSize:11}}>● {t.readyReview || 'Ready for review'}</strong>
    </div>
  </section>
  <section style={{...box,marginBottom:6,display:'flex',gap:7,alignItems:'center',overflowX:'auto',flex:'0 0 auto'}}>{files.map((x,i)=><button key={i} onClick={()=>{setActive(i);setView('editor');setResult('');setAudit(null);setAnalysis(null);setStatus(null);}} style={{...btn(i!==active),whiteSpace:'nowrap'}}><FileCode2 size={14}/>{x.fileName}</button>)}<button style={btn(false)} onClick={addFile}><Plus size={15}/>{t.addFiles||'Add file'}</button>{files.length>1&&<button style={btn(false)} onClick={removeFile}><Trash2 size={15}/></button>}</section>
  {view==='editor'&&<div style={{display:'flex',flexDirection:'column',flex:'1 1 auto',minHeight:0,overflow:'hidden'}}><section style={{...box,marginBottom:6,flex:'0 0 auto',display:'grid',gridTemplateColumns:'1.4fr 1.2fr 0.8fr 0.6fr',gap:10,width:'100%',boxSizing:'border-box'}}><label>{t.fileNameLabel || 'File name'}<input value={f.fileName} onChange={e=>update('fileName',e.target.value)} style={input}/></label><label>{t.workspacePathLabel || 'Workspace path'}<input value={f.filePath||''} onChange={e=>update('filePath',e.target.value)} style={input}/></label><label>{t.fileTypeLabel || 'Type'}<select value={detectFileType(f.fileName)} onChange={e=>update('fileName',f.fileName.replace(/\.[^.]+$/,'')+'.'+({javascript:'js',typescript:'ts',jsx:'jsx',tsx:'tsx',python:'py',java:'java',c:'c',h:'h',cpp:'cpp',hpp:'hpp',go:'go',json:'json',html:'html',css:'css'}[e.target.value]||'txt'))} style={input}>{FILE_OPTIONS.map(([v,n])=><option key={v} value={v}>{n}</option>)}</select></label><label>{t.charsLabel || 'Chars'}<div style={metric}>{sizeInfo(f.content).chars.toLocaleString()} / {LIMITS.maxSourceChars.toLocaleString()}</div></label></section>
  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,width:'100%',boxSizing:'border-box',flex:'1 1 auto',minHeight:0,overflow:'hidden'}}><section style={{...box,minWidth:0,display:'flex',flexDirection:'column',minHeight:0,overflow:'hidden'}}><h3 style={{margin:'0 0 4px',fontSize:13,flex:'0 0 auto'}}>{t.originalEditor || 'Original / Editor'}</h3><textarea value={f.content} onChange={e=>{update('content',e.target.value);setResult('')}} style={{...mono,flex:'1 1 auto',minHeight:0,overflow:'auto'}}/><input ref={inputRef} hidden type="file" onChange={e=>{const file=e.target.files?.[0];if(file){file.text().then(s=>{update('content',s);update('fileName',file.name)})}}}/><button style={{...btn(false),flex:'0 0 auto',marginTop:4}} onClick={()=>inputRef.current?.click()}><Upload size={15}/>{t.uploadFile || 'Upload file'}</button></section><section style={{...box,minWidth:0,display:'flex',flexDirection:'column',minHeight:0,overflow:'hidden'}}><h3 style={{margin:'0 0 4px',fontSize:13,flex:'0 0 auto'}}>{t.patchSectionTitle || 'SEARCH / REPLACE Patch'}</h3><textarea value={f.patchText} onChange={e=>update('patchText',e.target.value)} style={{...mono,flex:'1 1 auto',minHeight:0,overflow:'auto'}}/><div style={{...metric,flex:'0 0 auto'}}>{t.blocksMetric || 'Blocks'}: {parsed.blocks.length} • {t.errorsMetric || 'Errors'}: {parsed.errors.length}</div></section></div>
  <div style={{display:'flex',alignItems:'center',gap:10,marginTop:6,flexWrap:'wrap',flex:'0 0 auto'}}>
    <button disabled={busy} onClick={run} style={{flex:1,height:38,border:0,borderRadius:14,cursor:'pointer',fontWeight:800,color:'#17362e',background:'linear-gradient(135deg,#efca94,#c99560)',boxShadow:'0 6px 18px rgba(204,155,96,.20)',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:6}}><Play size={17}/>{busy?'Running…':t.run}</button>
  ...
  {result&&<section style={{...box,marginTop:12}}><div style={{display:'flex',justifyContent:'space-between',gap:8}}><h2>Approved Result</h2><div><button style={btn(false)} onClick={copy}><Copy size={14}/>{copied?'Copied':t.copy}</button><button style={btn(false)} onClick={download}><Download size={14}/>{t.download}</button></div></div><textarea readOnly value={result} style={{...mono,maxHeight:'40vh',overflow:'auto'}}/>{audit&&<details style={{marginTop:12}}><summary style={{cursor:'pointer',fontSize:12,color:C.sub,padding:'6px 0'}}>📋 عرض تقرير التدقيق الفني (JSON Audit Report)</summary><pre style={pre}>{JSON.stringify(audit,null,2)}</pre></details>}</section>}
 </div></main>
}
function AnalysisPanel({analysis,t}){const isAr = document.documentElement.dir === 'rtl';if(!analysis)return <section style={box}><Bug/> {t?.loadFirstToAnalyze || 'Load a file first to run analysis.'}</section>;return <section style={box}><h2 style={{margin:'0 0 12px',display:'flex',alignItems:'center',gap:8}}><Bug size={18}/> {t?.staticIntelligenceTitle || 'Static Intelligence'}</h2><div style={cards}><Metric n={t?.diagnosticsMetric || 'Diagnostics'} v={analysis.typeCheck.diagnostics.length}/><Metric n={t?.taintSources || 'Taint sources'} v={analysis.taint.sources}/><Metric n={t?.taintSinks || 'Taint sinks'} v={analysis.taint.sinks}/><Metric n={t?.taintMetric || 'Taint flows'} v={analysis.taint.findings.length}/></div><h3 style={{fontSize:14,margin:'14px 0 8px'}}>{t?.typeConsistencyTitle || 'Type / consistency diagnostics'}</h3>{analysis.typeCheck.diagnostics.map((d,i)=><div key={i} style={row(d.severity==='error'?C.red:C.orange)}>{d.code} — {isAr ? 'سطر' : 'line'} {d.line}: {d.message}</div>)}{analysis.typeCheck.diagnostics.length===0&&<div style={row(C.green)}>{t?.noTypeDiagnostics || 'No static type/consistency diagnostics.'}</div>}<h3 style={{fontSize:14,margin:'14px 0 8px'}}>{t?.taintFlowTitle || 'Data-flow / taint'}</h3>{analysis.taint.findings.map((d,i)=><div key={i} style={row(C.red)}>{d.code} — {isAr ? 'مصدر' : 'source'} {d.sourceLine} → {isAr ? 'هدف' : 'sink'} {d.sinkLine}: {d.message}</div>)}{analysis.taint.findings.length===0&&<div style={row(C.green)}>{t?.noTaintFlows || 'No source-to-sink flow detected by current rules.'}</div>}<div style={{marginTop:12,color:C.sub,fontSize:12}}>{t?.intelligenceNote||'Static analysis only; code is never executed.'}</div></section>}
