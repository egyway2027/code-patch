...
function send(res,status,body,req=null){
  ...
  const headers = {'content-type':'application/json; charset=utf-8','cache-control':'no-store','access-control-allow-origin':corsOrigin(req||{headers:{}}),'access-control-allow-methods':'GET,POST,OPTIONS','access-control-allow-headers':'content-type,authorization','access-control-max-age':'86400','vary':'Origin'};
  if(res.headersSent){ try{res.end(status===204?'':JSON.stringify(body));}catch{} return; }
...
async function main(req,res){
 ...
 const hostHeader = req.headers.host || (HOST.includes(':') && !HOST.startsWith('[') ? `[${HOST}]` : HOST);
 ...
 if(url.pathname==='/python-ast' || url.pathname==='/api/python-ast'){if(String(b.code??'').length>2_000_000)return send(res,413,{ok:false,message:'Python source exceeds safety limit.'},req);const {spawn}=await import('node:child_process');const helper=fileURLToPath(new URL('../api/python-ast.py',import.meta.url));const result=await new Promise(resolve=>{const child=spawn(PYTHON_CMD,[helper],{stdio:['pipe','pipe','pipe'],windowsHide:true});let out='',err='',done=false;const finish=x=>{if(!done){done=true;resolve(x)}};const timer=setTimeout(()=>{child.kill('SIGKILL');finish({ok:false,error:'Python AST helper timed out.'})},30000);child.stdout.setEncoding('utf8');child.stderr.setEncoding('utf8');child.stdout.on('data',x=>out+=x);child.stderr.on('data',x=>err+=x);child.stdin.on('error',()=>{});child.on('error',e=>{clearTimeout(timer);finish({ok:false,error:e.message})});child.on('close',code=>{clearTimeout(timer);if(done)return;if(code!==0)return finish({ok:false,error:(err||out||`exit ${code}`).trim()});try{finish(JSON.parse(out))}catch{finish({ok:false,error:'Invalid Python AST helper response.'})}});child.stdin.end(JSON.stringify({code:String(b.code??''),file_name:String(b.file_name||b.fileName||'file.py')}));});const status=result.strength==='ast'?200:(result.ok===false?500:200);return send(res,status,result,req)}
 ...
 if(url.pathname==='/commit'){const id=String(b.transactionId||''),record=await loadRecord(id);if(!record)return send(res,404,{ok:false,message:'Prepared transaction not found.'},req);const r=await commitPreparedTransaction(record,{workspaceRoot:WORKSPACE_ROOT,journalDir:JOURNAL_DIR,rejectExternalMutation:record.policy?.transaction?.rejectExternalMutation!==false});if(r.ok)await persistTransactionRecord(JOURNAL_DIR,r);else await persistTransactionRecord(JOURNAL_DIR,{...record,...r});return send(res,r.ok?200:409,r,req)}
 if(url.pathname==='/rollback'){const id=String(b.transactionId||''),record=await loadRecord(id);if(!record)return send(res,404,{ok:false,message:'Transaction not found.'},req);const r=await rollbackCommittedTransaction(record,{workspaceRoot:WORKSPACE_ROOT,journalDir:JOURNAL_DIR});return send(res,r.ok?200:409,r,req)}
